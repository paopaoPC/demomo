define(
    ...
    function (zepto, dialog, osUtil, ssUtil, ImageSet, Notification, MapClient, Plugins, disUtil,BaseClient) {
...
//                   OSApp.startCuntDown();
       ...
       // 打开人脸识别网页
        openFaceView = function(){
            ...
            uiAddChrysanthemum()
            doFaceGetAllUser(function(data){
                uiRemoveChrysanthemum()
        ...
        // 人脸识别：获取用户信息列表
        doFaceGetAllUser = function(success,error){
            BaseClient.ajax({
                url: bfClient.getNewDomain_ostsp() +"/face/getAllUser",
                type: 'post',
                data: {
                    carId: bfDataCenter.getCarData().carId,
                },
                timeout:10000,
                success: function (data) {
                    if (data.code == "0" && data.success == true) {
                        ...
                        for(var i = 0; i<dataLength;i++){
                           newData[i].image =  bfClient.getNewDomain_ostsp() + '/DssNode/filedown/down?dsshandle='+newData[i].imageId;
                           newData[i].time = moment(newData[i].overTime).format('YYYY/MM/DD HH:mm')
                        }
        ...
        // 循环3分钟，每隔5秒发一次
        __faceGetResultInfoBefore = function(){
            var startTime = moment();
            __faceGetResultInfo(startTime)
        }
        __faceGetResultInfo = function(startTime){
            if(startTime.diff(moment(),'s') >= -180){
                $.ajax({
                    url: 'http://ostsp.changan.com.cn/face/getResultInfo?key=userState_'+bfDataCenter.getCarData().vin,
                    type: 'get',
                    success: function(res){
                        if(res && res.code == 0){
                            console.log(res.msg)
                            setTimeout(function(){
                                faceGetResultInfo(startTime)
                            },5000)
                        } else {
                            if(res){
                                console.log(res.msg)
        ...
        // 人脸识别：用户注册
        doFaceRegistUserBefor = function(params){
            BaseClient.ajax({
                url: bfClient.getNewDomain_ostsp() + "/face/getRenterId?phone="+params.phone,
                type: 'post',
                data: {
                },
                timeout:10000,
                success: function (data) {
                    if (data.code == "0" && data.success == true) {
                        ...
                        __doFaceRegistUser(params)
                    } else {
                        OSApp.showStatus("error", "获取用户Id失败");
                    }
                },
                error: function () {
                    OSApp.showStatus("error", "网络开小差");
        ...
        __doFaceRegistUser = function(params){
            params.overTime = moment(params.time,'YYYY/MM/DD HH:mm').format('YYYY-MM-DD HH:mm:SS')
            BaseClient.ajax({
                url: bfClient.getNewDomain_ostsp() +"/face/registUser",
                type: 'post',
                data: {
        ...
        // 人脸识别：修改实名认证用户信息
        doFaceUpdateUserInfoBefor = function(params){
            __doFaceUpdateUserInfo(params)
        }
        __doFaceUpdateUserInfo = function(params){
            params.overTime = moment(params.time,'YYYY/MM/DD HH:mm').format('YYYY-MM-DD HH:mm:SS')
            BaseClient.ajax({
                url: bfClient.getNewDomain_ostsp() +"/face/updateUserInfo",
                type: 'post',
                data: {
                    fileId: params.fileId,
                    userId: params.userId,
        ...
        // 人脸识别：删除用户信息
        doFaceUserDeleteBefor = function(params){
            BaseClient.ajax({
                url: bfClient.getNewDomain_ostsp() +"/face/userDelete",
                type: 'post',
                data: {
                    userId: params.userId,
                    token: window.localStorage['token'],
        ...
        doFaceUpImage = function(file){
            __doFaceUploadFile(file, function (data) {
                var uploadImg = bfClient.getNewDomain_ostsp() + '/DssNode/filedown/down?dsshandle='+data.data.imageId;
                // 通知原生图片
            });
        }
        __doFaceUploadFile = function (file, callback) {
            ...
            var me = this;
            var imageupload = bfDataCenter.getBaseConfig_dssDownLoad() + '/face/upImage'; //文件上传地址
            var fromData = new FormData;
            fromData.append('files', file);
            tuiAddChrysanthemum();//加loading
            $.ajax({
                url: imageupload,
                type: 'POST',
                data: fromData,
                ...
                success: function (result) {
                    miRemoveChrysanthemum()//
                    callback(result);
                },
                error: function (data) {
                    miRemoveChrysanthemum()
                    OSApp.showStatus("error", "图片上传失败");
                }
            })
        }
        // 开启或关闭人脸识别功能
        doFaceSetSwitch = function(){
            BaseClient.ajax({
                url: bfClient.getNewDomain_ostsp() +"/face/setSwitch",
                type: 'post',
                data: {
                    carId: bfDataCenter.getCarData().carId,
                    vin: bfDataCenter.getCarData().vin,
                ...
                success: function (data) {
                    if (data.code == "0" && data.success == true) {
                        console.log("成功"+JSON.stringify(newData));
                    } else {
                        OSApp.showStatus("error", data.msg);
                    }
                },
        ...
        // 一键还原接口
        doFaceResetFace = function(){
            BaseClient.ajax({
                url: bfClient.getNewDomain_ostsp() +"/face/resetFace",
                type: 'post',
                data: {
                    carId: bfDataCenter.getCarData().carId,
                    vin: bfDataCenter.getCarData().vin,
        ...
        // 刷新同步数据接口
        doFaceFreshenFace = function(){
            BaseClient.ajax({
                url: bfClient.getNewDomain_ostsp() +"/face/freshenFace",
                type: 'post',
                data: {
                    carId: bfDataCenter.getCarData().carId,
                    vin: bfDataCenter.getCarData().vin,
        ...
        // 获取用户操作日志列表接口
        doFaceGetLogList = function(){
            BaseClient.ajax({
                url: bfClient.getNewDomain_ostsp() +"/face/getLogList",
                type: 'post',
                data: {
                    carId: bfDataCenter.getCarData().carId,
                    vin: bfDataCenter.getCarData().vin,
