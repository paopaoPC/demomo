...
define(['shared/timePicker/js/iscroll',
    'moment',
    'css!shared/timePicker/css/common'
    ...
    var DatePick = function($) {
        $.fn.date = function(options, Ycallback, Ncallback) {
            ...
            $.fn.date.defaultOptions = {
                    ...
                    theme: "date", //控件样式（1：日期，2：日期+时间,3:时间）
                    mode: null, //操作模式（滑动模式）
            ...
            function refreshTime() {
                ...
                //     SecondScroll.scrollTo(0, 40, 0, true); //显示“下午”
                //     initH = initH - 12;
                // }
                HourScroll.scrollTo(0, initH * 40, 0, true);
                MinuteScroll.scrollTo(0, initI * 40, 0, true);
                initH = parseInt(nowdate.getHours());
            }
            ...
            function init_iScrll() {
                ...
                yearScroll = new iScroll(yearwrapperEl, {
                    ...
                    onScrollEnd: function() {
                        ...
                        var day = indexD > 9 ? indexD : "0" + indexD;
                        thatParents.find("#d" + day).addClass("activeDay");
                        dayScroll.refresh();
                    }
                });
            ...
            function showTime(){
                init_iScroll_datetime();
                addTimeStyle1();
                thatParents.find("#datescroll").hide();
                thatParents.find("#datemark span").hide();
                thatParents.find("#datetitle").text("请选择时间");
                thatParents.find("#datescroll_datetime").show();
                thatParents.find("#Hourwrapper ul").html(createHOURS_UL());
                thatParents.find("#Minutewrapper ul").html(createMINUTE_UL());
                // thatParents.find("#Secondwrapper ul").html(createSECOND_UL());
            ...
            function init_iScroll_datetime() {
                ...
                HourScroll = new iScroll(thatParents.find('#Hourwrapper')[0], {
                    snap: "li",
                    vScrollbar: false,
                    onScrollEnd: function() {
                        indexH = Math.round((this.y / 40) * (-1)) + 1;
                        thatParents.find('#Hourwrapper li').removeClass('activeHour')
                        thatParents.find('#Hourwrapper li').eq(indexH).addClass('activeHour')
                        // HourScroll.refresh();
                    }
                })
                MinuteScroll = new iScroll(thatParents.find('#Minutewrapper')[0], {
                    snap: "li",
                    vScrollbar: false,
                    onScrollEnd: function() {
                        indexI = Math.round((this.y / 40) * (-1)) + 1;
                        thatParents.find('#Minutewrapper li').removeClass('activeHour')
                        thatParents.find('#Minutewrapper li').eq(indexI).addClass('activeHour')
                        // HourScroll.refresh();
                    }
                })
                SecondScroll = new iScroll(thatParents.find('#Secondwrapper')[0], {
                    snap: "li",
                    vScrollbar: false,
                    onScrollEnd: function() {
                        indexS = Math.round((this.y / 40) * (-1));
                        // thatParents.find('#Secondwrapper li').removeClass('activeHour')
                        // thatParents.find('#Secondwrapper li').eq(indexS).addClass('activeHour')
                        HourScroll.refresh();
                    }
                })
            }
            ...
            function addTimeStyle1(){
                thatParents.find("#Hourwrapper").css("top","62px");
                thatParents.find("#Minutewrapper").css("top","62px");
                thatParents.find("#Secondwrapper").css("top","62px");
                // thatParents.find("#Secondwrapper ul").css("margin-top","10px");
            }
                //创建 --年-- 列表
