define(['shared/js/client', "butterfly/extend", "shared/js/notification", "client/control-client", "main/Plugins"], function (BaseClient, extend, Notification, control, Plugin) {
    ...
    cls.winmu_getCheckCode = function(params){
        var url = this.getNewDomain_ostsp() + '/appService/btKey/getCheckCode';
    ...
    cls.winmu_syncUser = function(params){
        var url = this.getNewDomain_ostsp() + '/appService/btKey/syncUser';
    ...
    cls.winmu_bindCar = function(params){
        var url = this.getNewDomain_ostsp() + '/appService/btKey/bindCar';
    ...
    cls.winmu_getSN = function(params){
        var url = this.getNewDomain_ostsp() + '/appService/btKey/getSN';
    ...
    cls.getUserAppData_control = function(data){
        var postData = {
        ...
        var itemData = {
            appversion: window._zhihe_app_version,
            systemversion: window._zhihe_OS_version,
            timestamp: moment(postData.timestamp).valueOf(),
            userid: window.localStorage['userId']
        }
        itemData.item = data.item;
        itemData.status = data.status;
        itemData.vin = data.vin;
        postData.data.data.push(itemData);
        postData.data = JSON.stringify(postData.data)

       // var url = this.getNewDomain_ostsp() + '/appdataHbase/getUserAppDataTest';
       var url =  'http://ostspdev.changan.com.cn:9000/appdataHbase/getUserAppDataTest';
       // var url = this.getNewDomain_ostsp() + '/appdataHbase/getUserAppData';
    ...
    cls.getUserAppData_shared = function(data){
        var postData = {
           timestamp:  moment().format('YYYY-MM-DD HH:mm:ss'),
           token: window.localStorage['token'],
           data: {
            "catagray": "bt_key_share",
            "data": []
           }
        };
        ...
        itemData.item = '蓝牙钥匙分享';
        itemData.shareusernumber = data.shareusernumber;
        itemData.vin = data.vin;
        postData.data.data.push(itemData);
        postData.data = JSON.stringify(postData.data)
    ...
    cls.upIosModel = function(params){
        var postData = {
           timestamp:  moment().format('YYYY-MM-DD HH:mm:ss'),
           token: window.localStorage['token'],
           data: JSON.stringify(params.data)
        };
        var url = 'http://ostsp.changan.com.cn/rabbitMq/getUserBehavior';
        $.ajax({
            url: url,
            type: 'POST',
            data: postData,
