(function(root, factory) {
  ...
  //
	...
	$(function(){
		...
		if(path) {
			PP_require([path], function(MyApp) {
				if(!_.isFunction(MyApp) || !_.isFunction(MyApp.prototype.fly)) {
					var error = 'error: 锟皆讹拷锟斤拷Application锟斤拷锟斤拷锟紹utterfly.Application锟教筹拷';
					console.log(error);
					throw new Error(error);
				}else {
					root.butterfly = new MyApp(document.body);
					root.butterfly.fly();
				}
			});
		}else {
			root.butterfly = new Application(document.body);
			root.butterfly.fly();
		}
	});
