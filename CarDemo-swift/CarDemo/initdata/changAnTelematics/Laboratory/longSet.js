...
define(
    ...
    function (template, View, Butterfly, ssUtil,iscroll, disUtil,Notification,LabClient, dialog, codeInterface)
    {
        ...
        cls.posGenHTML = function()
        {
            ...
            if (bfDataCenter.getUserSessionValue('canGetImage-long-set') == 'false') {
                node.addClass("active");
                this.$('input').attr('disabled',true)

                this.getElement(".content-image").html("<div style='line-height:100px'>正在修改远程设置中~~</div>");
            }else{
        ...
        cls.onConfirmCarLocation = function(pin)
        {
            ...
            var operate = "PortableFunctionSet";
            var phoneType = "";
            var phoneId   = "";
            var speedLatch = this.$("input[name='latch']:checked").val();
            var parkingUnlock = this.$("input[name='parkingUnlock']:checked").val();
            var lightTimeGoHome = this.$("input[name='backHome']:checked").val();
            var sunroofAutomaticShutdown = this.$("input[name='skylight']:checked").val();
            var automaticWiperOfReverseGear = this.$("input[name='Wiper']:checked").val();
            if(window.device)
            {
            ...
            bfClient.controlCar({
                data:{'cmd':operate,
                    ...
                    'carId':bfDataCenter.getCarId(),
                    'pin':pin,
                    speedLatch: speedLatch,
                    parkingUnlock: parkingUnlock,
                    lightTimeGoHome: lightTimeGoHome,
                    sunroofAutomaticShutdown: sunroofAutomaticShutdown,
                    automaticWiperOfReverseGear: automaticWiperOfReverseGear,
                    // 'notifyTuType':1
                },
                ...
                success:function(data)
                {
                    if(data.code == 0 && data.success)
                    {
                        ...
                        var imageNode = me.$(".bottom-item");
                        imageNode.addClass("active");
                        me.$('input').attr('disabled',true)

                        imageNode.unbind("click");
                        bfDataCenter.setUserSessionValue('canOperate','false');
                        me.$(".content-image").html("<div style='line-height:100px'>正在修改远程设置中~~</div>");
                            ...
                            bfClient.getControlInfoByTaskId({
                                ...
                                success:function(data){
                                    ...
                                    if(data.handleStatus == "Completed"){
                                        ...
                                        bfDataCenter.setUserSessionValue('canOperate','false');
                                        bfDataCenter.setUserSessionValue('canGetImage-long-set','false');
                                    }else{
                                        //让远程控制可操作
                                        me.controlError();
                                    }
                        ...
                        bfDataCenter.setUserSessionValue('canOperate', 'true');
                        bfDataCenter.setUserSessionValue('canGetImage-long-set', 'true');
                        Notification.show({
                            type:'error',
                            message:data.msg
                        });
        ...
        cls.controlComplete = function(taskId){
            var me = this;
            var imageNode = this.$(".bottom-item");
            this.$(".content-image").html("<div>修改成功~~</div>");
            me.getImage(taskId);
            bfDataCenter.setUserSessionValue('canOperate','true');
            bfDataCenter.setUserSessionValue('canGetImage-long-set','true');
        ...
        cls.controlError = function(){
            ...
            bfDataCenter.setUserSessionValue('canOperate','true');
            bfDataCenter.setUserSessionValue('canGetImage-long-set','true');
            this.$(".content-image").html("<div>修改失败~~</div>");
            imageNode.removeClass("active");
            this.$('input').attr('disabled',false)
        ...
        cls.getImage = function(taskId){
            ...
            bfClient.getImage({
                ...
                success: function(data){
                    if (data.code == 0) {
                        var imageNode = me.$(".content-image");
                        imageNode.empty();
