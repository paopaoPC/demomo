define(
    [
        "text!bluetooth/shared-list-item.html",
        "common/navView",
        'butterfly',
    ...
    function (template, View, Butterfly, DatePicker, Notification,Swiper,moment) {
        ...
        var cls =
        {
            id: 'bluetoothSharedListItemView',
            html: template,
            events: {
        ...
        cls.posGenHTML=function(){
            ...
            me.startTime = moment(this.itemData.startTime).format('YYYY-MM-DD HH:mm')
            me.$('#item-input-1').html(me.startTime)
            me.endTime = moment(this.itemData.expireTime).format('YYYY-MM-DD HH:mm')
            me.$('#item-input-2').html(me.endTime)
            me.$('#nickName').val(this.itemData.nickName)
            me.$('#phone-input').val(this.itemData.phone)
            me.bindDatePicker()
        };
        cls.onShow = function () {
            var me = this;
        ...
        cls.onShared = function(){
            ...
            // }
            me.uiAddChrysanthemum()
            window.modify_blueTooth_bluetooth_shared_1_asdsadf = function(data){
                if(data.code == '502' && data.msg == '令牌错误'){
            ...
            OSApp.modifyPermissionWithOtherPhone_blueTooth(JSON.stringify({
				userPhone:me.phone,
                phone: phone,
                vin: me.vin,
                nickName: nickName,
                ...
                token: window.localStorage['token']
            }),'modify_blueTooth_bluetooth_shared_1_asdsadf')
