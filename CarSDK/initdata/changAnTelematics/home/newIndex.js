...
define(
    ...
    // function (Backbone,template,View,Toolkit,ScrollPullDown,Swiper,Notification,dialog,oldDialog,Air,autoAir,Remote,CarWindows,controlIndex,ssUtil,iscroll,controlCmd ,CarResource,progress) {
        ...
        cls.posGenHTML = function () {
            ...
            window.__getBlueToothState = function(data){
                if(window.__getBlueToothStateByBlueeIndexView){
                    ...
                    window.threeDDD.contentWindow.openDoor('qlc', data.leftFrontDoorStatus?1:0);   //车门打开状态
                    window.threeDDD.contentWindow.openDoor('hlc', data.leftRearDoorStatus?1:0);
                    window.threeDDD.contentWindow.openDoor('qrc', data.rightFrontDoorStatus?1:0);
            ...
            if(!me.setIntervalKey){
                window.getConnectStatus_blueTooth_home_index_22 = function(data){
                    if(data.data == "true" || data.data == '1'){
                        ...
                        if(deviceIsIOS && me.$('.bluetoothPasscode span').html() && me.bluetoothStaus && me.isBlueToothCar){
                            me.$('.bluetoothPasscode').show();
                        }
        ...
        // cls.upUserData = function(){
        //     // 每个月只掉一次
        //     if(!navigator.packaging || !navigator.packaging.upUserData){
        //         return
        //     }
        //     var userMobile = window.localStorage['userMobile'];
        //     if( window.localStorage['upUserDataStaus'+userMobile] != 'true' || window.localStorage['upUserDataTime'+userMobile] != moment().format('YYYYMM')){
        //         // console.log('你好呀');
        ...
        //                 phone:bfDataCenter.getUserData().myInfo.mobile,
        //                 url:bfClient.getUpUserDataDomain_ostsp()

        //             }
        //             console.log(JSON.stringify(data));
        //             navigator.packaging.upUserData(data,function(result){
        //                 if(result === 'true'){
        //                     window.localStorage['upUserDataStaus'+userMobile] = 'true';
        //                     window.localStorage['upUserDataTime'+userMobile] = moment().format('YYYYMM');
        //                 }
        //             })
        //         },15000)
        //     }
                    ...
                    switch(data){
                        ...
                        case 'actionLock_blueTooth':
                            window.actionLock_blueToothCallback_bluetooth_index = function(data){
                                home.uiRemoveChrysanthemum_blueetooth()
                                if(data.code != '200'){
                                    if($('.notification').html() != '未连接，不能发送指令'){
                                        Notification.show({
                                            type:'info',
                                            message: '未连接，不能发送指令',
                                        })
...
//                            OSApp.openCarLocationView();//true为TBox车，false为非tBox
        ...
        cls.updateanimation = function (data) {   //控制模型动画接口  需传入后台获取的车辆控制状态data
            var me = this;
