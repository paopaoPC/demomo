...
/******/ 	}
/******/ 	var parentHotUpdateCallback = window["webpackHotUpdatedatePicker"];
/******/ 	window["webpackHotUpdatedatePicker"] = 
...
/******/ 	var hotCurrentHash = "1270a1a8ce6ff39f726b"; // eslint-disable-line no-unused-vars
...
var DEFAULT_OPTIONS = {
    ...
    interval: 1,
    initStrDate: '',
    callBack: function callBack(timeStr, timeStamp) {
...
function datePicker() {
    ...
    var callBack = CHOICE_OPTIONS.callBack,
        ...
        $pickerWrap = CHOICE_OPTIONS.$pickerWrap;
    ...
    filter.day = function () {
        var timeStamp = Date.now(),
            daysArr = [];
        for (var i = after_day; i < app_day; i++) {
            ...
            var text = ['今天', '明天', '后天', month + '\u6708' + day + '\u65E5'][i] || month + '\u6708' + day + '\u65E5';
    ...
    filter.hour = function () {
        ...
        if (current.hour < hour_area[0]) {
            pre_min = 0;
        }

        for (var i = current.hour; i < 24; i++) {
            hoursArr.push(i);
    ...
    filter.minute = function () {
        var minutesArr = [];
        for (var i = Math.ceil(current.minute / dis_min) * dis_min + pre_min; i < 60; i += dis_min) {
            minutesArr.push(i);
            start.minuteArr.push(i);
        }

        if (!minutesArr.length) {
            for (var _i = Math.ceil((current.minute + pre_min - 60) / dis_min) * dis_min; _i < 60; _i += dis_min) {
                minutesArr.push(_i);
                start.minuteArr.push(_i);
            }
        }
        return minutesArr;
    }();
    ...
    var initData = function () {
        for (var h = hour_area[0]; h <= hour_area[1]; h++) {
            original.hour.push(h);
        }
        for (var m = 0; m < 60; m += dis_min) {
            original.minute.push(m);
        }

        if (after_day) {
            filter.hour = JSON.parse((0, _stringify2.default)(original.hour));
            filter.minute = JSON.parse((0, _stringify2.default)(original.minute));
        }
    ...
    var wheel = $pickerWrap.querySelectorAll('.wheel-scroll'),
        wheelDay = wheel[0],
        wheelHour = wheel[1],
        wheelMinute = wheel[2];

    datePicker._setPicker = function (whell, index) {
        new _datePicker2.default(whell, index);
    };
    ...
    new _datePicker2.default(wheelDay, 0);
    new _datePicker2.default(wheelHour, 0);
    new _datePicker2.default(wheelMinute, 0);

    Object.defineProperty(wheelDay, 'index', {
        set: function set(value) {
            if (value !== 0 || after_day) {
                ...
                var hourIndex = original.hour.indexOf(selected.hour);
                var minuIndex = original.minute.indexOf(selected.minute);
                new _datePicker2.default(wheelHour, hourIndex);
                new _datePicker2.default(wheelMinute, minuIndex);
            } else {
                createHTML(wheelHour, filter.hour, '点');
                start.hourArr = filter.hour;
                var index = selected.hour < start.hour ? 0 : filter.hour.indexOf(selected.hour);
                new _datePicker2.default(wheelHour, index);
                selected.hour = filter.hour[index];

                if (index === 0) {
                    var _minuIndex = selected.minute < start.minute ? 0 : filter.minute.indexOf(selected.minute);
                    createHTML(wheelMinute, filter.minute, '分');
                    start.minuteArr = filter.minute;
                    new _datePicker2.default(wheelMinute, _minuIndex);
                    selected.minute = filter.minute[_minuIndex];
                } else {
                    var _index = original.minute.indexOf(selected.minute);
                    createHTML(wheelMinute, original.minute, '分');
                    start.minuteArr = original.minute;
                    new _datePicker2.default(wheelMinute, _index);
                }
            }
        }
    });
    ...
    Object.defineProperty(wheelHour, 'index', {
        set: function set(value) {
            ...
            if (value !== 0 || wheelDay.current || after_day) {

                var index = original.minute.indexOf(selected.minute);
                createHTML(wheelMinute, original.minute, '分');
                start.minuteArr = original.minute;
                new _datePicker2.default(wheelMinute, index);
            } else {

                var _index2 = selected.minute < start.minute ? 0 : filter.minute.indexOf(selected.minute);
                selected.minute = filter.minute[_index2];
                createHTML(wheelMinute, filter.minute, '分');
                start.minuteArr = filter.minute;
                new _datePicker2.default(wheelMinute, _index2);
            }
        }
    });
    ...
    var confirmTime = function confirmTime(e) {
        e.preventDefault();
        e.stopPropagation();
        var index = wheelDay.current || 0;
        var minute = selected.minute,
            hour = selected.hour,
    ...
    $pickerWrap.querySelector('.confirm').addEventListener('touchend', confirmTime, false);
    $pickerWrap.querySelector('.cancel').addEventListener('touchend', toggle, false);
...
var datePicker = function () {
    function datePicker(obj, initIndex, callback) {
    ...
    (0, _createClass3.default)(datePicker, [{
        ...
        key: 'bindEvent',
        value: function bindEvent(selector) {
            var _this = this;
            ...
            var touchmove = function touchmove(e) {
                ...
                if (_this.index == 0 && iDisY > 0 || _this.index == -length && iDisY < 0) {
                    step = 1 - Math.abs(iDisY) / selector.clientWidth;
                    step = Math.max(step, 0);
                    iDisY = parseInt(iDisY * step);
                }
                datePicker.setTranslate3d(selector, _this.index * _this.distance + iDisY);
                datePicker.setRotateX(_this.obj.children, _this.index + iDisY / _this.distance);
            };
            var touchend = function touchend(e) {
                ...
                if (iDisY === 0) {
                    _this.index = -e.target.getAttribute('data-index');
                    selector.style.webkitTransitionDuration = '200ms';
                    selector.addEventListener("webkitTransitionEnd", function (f) {
                    ...
                    _this.obj.index = Math.abs(_this.index);
                    _this.obj.current = Math.abs(_this.index);
                    ...
                    timer = setInterval(function (f) {
                        if (Math.abs(speed) <= 1) {
                            clearInterval(timer);
                            if (flag) {
                                _this.index += Math.round(iDisY / _this.distance);
                            }
                            _this.index = _this.index > 0 ? Math.min(_this.index, 0) : Math.max(_this.index, -length);
                            _this.index = _this.index > 0 ? 0 : _this.index < -length ? -length : _this.index;
                            selector.style.webkitTransitionDuration = '400ms';
                            selector.addEventListener("webkitTransitionEnd", function (f) {
                                selector.style.webkitTransitionDuration = '0ms';
                            });
                            Array.prototype.slice.call(selector.children).forEach(function (ele) {
                                ele.style.webkitTransitionDuration = '200ms';
                                ele.addEventListener("webkitTransitionEnd", function (f) {
        ...
        value: function setRotateX(obj, index) {
            var deg = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 25;

            Array.prototype.slice.call(obj).forEach(function (ele, i) {
